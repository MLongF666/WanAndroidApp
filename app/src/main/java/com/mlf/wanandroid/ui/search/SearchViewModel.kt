...
class SearchViewModel : BaseViewModel() {
    ...
    fun getHistoryList(context: Context) {
        launch({
            ...
            Log.d(
                "SearchViewModel",
                "getHistoryList _searchHistoryList.value:${_searchHistoryList.value}"
            )
        }, {
            exception.value.apply {
                Log.e("SearchViewModel", "getHistoryList exception: $this")
            }
        })
    }
    ...
    fun searchList(page: Int, context: Context, content: String) {
        launch({
            handleRequest(SearchRepository.getArticleList(content, page), {
                ...
                Log.d(
                    "SearchViewModel",
                    "searchList page:$page content:$content _articleListLiveData.value:${_articleListLiveData.value}"
                )
            })
